...
{
	private readonly BlazorFilmContext _db;
	private readonly IMapper _mapper;

	public DbService(BlazorFilmContext db, IMapper mapper)
	{
		_db = db;
		_mapper = mapper;
	}
    ...
    public void IncludeReference<TEntity>()
    ...
    //reference
    public async Task<List<TDto>> GetReferenceAsync<TEntity, TDto>()
		where TEntity : class, IReferenceEntity
		where TDto : class
	{
		var entities = await _db.Set<TEntity>().ToListAsync();
	...
	public bool DeleteReference<TEntity, TDto>(TDto dto)
		where TEntity : class
		where TDto : class
	{
		try
